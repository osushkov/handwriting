...
template <typename R, typename Func> struct task_package_impl : public task_package {
  task_package_impl(Func &&func, std::promise<R> &&promise)
      : promise(std::forward<std::promise<R>>(promise)), func(std::forward<Func>(func)){};

  virtual void run() { promise.set_value(func()); }

  virtual void set_exception(std::exception_ptr except_ptr) { promise.set_exception(except_ptr); }

  std::promise<R> promise;
  Func func;
};

template <typename Func> struct task_package_impl<void, Func> : public task_package {
